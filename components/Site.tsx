...
function Hero() { return <section id="home" className="hero"><div className="container hero-grid"><div className="hero-copy"><Eyebrow>BETTER MAN / 男性日常状态保养</Eyebrow><h1>不够硬、撑不久、<br/><em>状态越来越难维持？</em></h1><p className="hero-value">年纪慢慢上来，精神、体力、耐力都不像以前。<br/>很多男人不是不想，是身体真的开始跟不上。</p><div className="hero-primary-action"><WaButton>WhatsApp 私下了解</WaButton><small>不懂怎么吃？直接 WhatsApp 问我们。</small></div></div><div className="hero-visual"><div className="hero-art"><div className="hero-product"><Image src="/images/hero/better-man-product.png" alt="Better Man 宝马糖产品盒与独立包装" width={1254} height={1254} priority className="product-image" sizes="(max-width: 650px) calc(100vw - 40px), (max-width: 900px) min(620px, calc(100vw - 48px)), 45vw"/></div></div></div><div className="hero-close"><p className="hero-slogan">Better Man，有心又有力，随时都 Ready.</p><a className="button button-secondary" href="#how-to-use">看看 Better Man 怎么吃 <ArrowDown size={17}/></a></div></div><div className="hero-bottom container"><span>从平时开始，照顾男人的状态。</span><a href="#state" aria-label="继续了解男性状态">向下探索 <ArrowDown size={15}/></a></div></section>; }
const problems = [
...
function CustomerReviews({ photos, videos }: { photos: PhotoReview[]; videos: VideoReview[] }) {
  ...
  return <section id="reviews" className="section customer-reviews"><div className="container">
    <div className="reviews-heading"><div><span>05 / REAL CUSTOMER FEEDBACK</span><strong>真实顾客反馈</strong></div><div><h2>不是我们说好，<br/><em>是他们真的用过。</em></h2></div></div>
    <div className="review-group"><div className="mobile-review-subhead"><span>CUSTOMER STORIES</span><h3>听他们自己怎么说。</h3></div><div className="review-track review-video-track">{videos.map((item) => <button key={`v${item.id}`} type="button" className="mixed-card mixed-video" disabled={!item.available} onClick={() => setActiveVideo(item)} aria-label={item.available ? `播放${item.title}` : `${item.title}素材待上传`}>
...
function FinalCTA() { return <section className="final-cta"><div className="container final-inner"><Eyebrow>BETTER MAN</Eyebrow><h2>想了解自己<br/><em>适不适合？</em></h2><p>直接 WhatsApp 一对一咨询。</p><WaButton light>WhatsApp 咨询 Better Man</WaButton><div className="final-slogan">Better Man｜有心又有力，随时都 Ready。</div></div></section>; }
