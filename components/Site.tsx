...
const benefits = [
  [HeartPulse, "硬度", "照顾男性硬度状态。"],
  [TrendingUp, "耐久", "支持耐力与持久状态。"],
  [Brain, "精神", "维持日常精神状态。"],
  [BatteryFull, "体力", "支持日常体力与活力。"],
  [Moon, "夜尿", "关注夜间频繁起床状态。"],
  [HeartPulse, "三高日常管理", "关注整体日常健康管理。"],
] as const;
function Benefits() { return <section id="benefits" className="section section-cream support-section"><div className="container"><div className="support-visual-head"><SectionHeading eyebrow="02 / WHAT BETTER MAN SUPPORTS" title="Better Man，照顾男人在意的状态。" desc="Better Man 照顾什么？"/><div className="support-person-image"><Image src="/images/lifestyle/daily-support.png" alt="成熟男性在办公室手持 Better Man 产品并比赞" width={1536} height={1024} loading="lazy" sizes="(max-width: 768px) calc(100vw - 40px), 52vw"/></div></div><div className="benefit-grid support-grid">{benefits.map(([Icon,label,desc], index) => <article className="benefit-item support-item" key={label}><span className="support-icon"><Icon size={23} strokeWidth={1.7}/></span><small>0{index + 1}</small><div><h3>{label}</h3><p>{desc}</p></div></article>)}</div></div></section>; }
